...
public:
    static unique_ptr<Student> createStudent(const string& type, string n, string d, string sN, string cN, int sc, vector<vector<Course>> sems) {
        if (type == "Uni" || type == "uni") {
            return make_unique<UniStudent>(n, d, sN, cN, sc, sems);
        } else if (type == "College" || type == "college") {
            return make_unique<CollegeStudent>(n, d, sN, cN, sc, sems);
        } else {
            return nullptr;
        }
    }
